...
class NombreClase(Aplicacion.Aplicacion):
	def iniciar(self,**args):	
		...
		self.vars["var1"] = Variable(args["param1"],self.modifGenerico,minimo=0,maximo=5,flags={"flag1":True}) 
		 # para agregar items al menu, crear la estructura de Nodos que se queira ,luego llamar agregarMenu(indice del menu,Nodo)
		leaf1 = Leaf("AccionJugosa","descripcion",self.estaEsLaFuncionJugosa)
		nodo1 = Nodo("asd","",Nodo("asd2","",Nodo("asd3","",Leaf("LEAF","",self.menu.expandir))))
		self.agregarMenu(0,leaf1)
		self.agregarMenu(-2,nodo1)
	...
	def estaEsLaFuncionJugosa(self):
		print "esta es la funcion que le da sentido a esta aplicacion :)"
...
if __name__ == '__main__':	
	a = NombreClase("NombreAPP","Version",False,param1=1,param2=2)
	a.menuPrincipal()		

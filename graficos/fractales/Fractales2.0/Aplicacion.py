...
class Aplicacion:
	...
	"""
	def __init__(self,appnombre,version,ratio,factorRatio,**args):
		...
		self.vars["ratio"]  = Variable(ratio,self.modifTamano,minimo=[1,1])
		self.vars["factorRatio"] = Variable(factorRatio,self.modifTamano,minimo=2)
		self.vars["colorFondo"] = Variable([0,0,0],self.modifColor,minimo=[0,0,0],maximo=[255,255,255])
		self.vars["filesPath"] = Variable(os.getcwd(),self.modifGenerico)
		...
		#Genero el menu
		self.menu = Nodo("--- " +self.appNombre + " " + self.version + " ---","----------------------",
	...
	def modifValoresPosibles(self,key):
		tipo = type(self.vars[key].valor)
		print "valores posibles: " + str(self.vars[key].valoresPosibles)
		self.vars[key].valor = validador.ingresar(tipo, validador.igual, self.vars[key].valoresPosibles)
